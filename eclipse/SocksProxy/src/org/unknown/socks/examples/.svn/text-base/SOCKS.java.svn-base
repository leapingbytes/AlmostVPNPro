...
import org.unknown.socks.server.IdentAuthenticator;
...
//      IdentAuthenticator auth = new IdentAuthenticator();
      ...
      if(args.length == 0){
         file_names = new URL[1];
         file_names[0] = ClassLoader.getSystemResource( "socks.properties" );
      }else{    	  
         file_names = new URL [ args.length ];
         for( int i = 0; i < args.length; i++ ) {
        	 file_names[ i ] = new File( args[ i ] ).toURL();
         }
      }

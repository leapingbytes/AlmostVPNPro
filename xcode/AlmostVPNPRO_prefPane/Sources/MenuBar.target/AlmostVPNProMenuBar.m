...
- (void) awakeFromNib {
    ...
    [_statusItem setImage: [ PathLocator imageNamed:  @"MenuBarEmpty" fromBundle: [ NSBundle mainBundle ]]];   
...
- (void) doServerEvent: (NSNotification*) notification {
	...
	if( record != nil && [ record isStateEvent ] ) {
		@synchronized ( self ) {	
			...
			[_statusItem setImage: [ PathLocator imageNamed: [ self statusToImageName ]  fromBundle: [ NSBundle mainBundle ] ]];
		}
	}	
	[ poll drain ];
}
...
- (void) doServerStateEvent: (NSNotification*) notification {
	...
	if( _serviceIsRunning != [[ notification object ] boolValue ] ) {
...
- (void) stopAllProfiles: (id) sender {
	NSArray* profiles = [ self listProfiles ];
	NSEnumerator* profileEnum = [ profiles objectEnumerator ];
	AlmostVPNProfile* profile;
	while(( profile = [ profileEnum nextObject ] ) != nil ) {
		[ ServerHelper stopProfile: [ profile name ]];
	}
}

- (void) pauseAllProfiles: (id) sender {
	NSArray* profiles = [ self listProfiles ];
	NSEnumerator* profileEnum = [ profiles objectEnumerator ];
	AlmostVPNProfile* profile;
	while(( profile = [ profileEnum nextObject ] ) != nil ) {
		if( [ @"running" isEqual: [ profile state ]] ) {
			[ ServerHelper pauseProfile: [ profile name ]];
		}
	}
}

- (void) resumeAllProfiles: (id) sender {
	NSArray* profiles = [ self listProfiles ];
	NSEnumerator* profileEnum = [ profiles objectEnumerator ];
	AlmostVPNProfile* profile;
	while(( profile = [ profileEnum nextObject ] ) != nil ) {
		if( [ @"paused" isEqual: [ profile state ]] ) {
			[ ServerHelper startProfile: [ profile name ]];
		}
	}
}

- (void) startAllProfiles: (id) sender {
	NSArray* profiles = [ self listProfiles ];
	NSEnumerator* profileEnum = [ profiles objectEnumerator ];
	AlmostVPNProfile* profile;
	while(( profile = [ profileEnum nextObject ] ) != nil ) {
...
//		NSString* stateIcon = [ _profileStates objectForKey: profileName ];
		...
		[ item setImage: [ PathLocator imageNamed: stateIcon ofSize: NSMakeSize( 12.0, 12.0 )  fromBundle: [ NSBundle mainBundle ]]];
		item = nil;			
	}
	[ menu addItem: [ NSMenuItem separatorItem ]];
	if( someIdle || somePaused ) {
		item = [[ NSMenuItem alloc ] initWithTitle: @"Start ALL" action: @selector( startAllProfiles: ) keyEquivalent: @"" ];
			[ item setTarget: self ];
			[ item setEnabled: YES ];
			[ menu addItem: item ];
		[ item release ];
	...
	if( somePaused ) {
		item = [[ NSMenuItem alloc ] initWithTitle: @"Resume ALL" action: @selector( resumeAllProfiles: ) keyEquivalent: @"" ];
			[ item setTarget: self ];
			[ item setEnabled: YES ];
			[ menu addItem: item ];
		[ item release ];
	...
	if( someRunning ) {
		item = [[ NSMenuItem alloc ] initWithTitle: @"Stop ALL" action: @selector( stopAllProfiles: ) keyEquivalent: @"" ];
			[ item setTarget: self ];
			[ item setEnabled: YES ];
			[ menu addItem: item ];
		[ item release ];
		item = [[ NSMenuItem alloc ] initWithTitle: @"Pause ALL" action: @selector( pauseAllProfiles: ) keyEquivalent: @"" ];
			[ item setTarget: self ];
			[ item setEnabled: YES ];
			[ menu addItem: item ];
		[ item release ];
	...
	[ menu addItem: [ NSMenuItem separatorItem ]];
		...
		SEL serviceSelector;
		if( _serviceIsRunning ) {
			serviceTitle = @"Restart AVPN Server";
			serviceSelector = @selector( restartAlmostVPNService: );
		} else {
			serviceTitle = @"Start AVPN Server";
		...
		[ item release ];
		if( _serviceIsRunning ) {
			item = [[ NSMenuItem alloc ] initWithTitle: @"Stop AVPN Server" action: @selector( stopAlmostVPNService: ) keyEquivalent: @"" ];
				[ item setTarget: self ];
				[ item setEnabled: YES ];
				[ menu addItem: item ];
			[ item release ];

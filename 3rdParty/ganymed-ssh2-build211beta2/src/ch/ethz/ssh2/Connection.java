 ...
 * @version $Id: Connection.java,v 1.31 2006/10/20 20:50:24 cplattne Exp $
 */
...
public class Connection
{
	 ...
	 */
	public final static String identification = "Ganymed Build_211beta2";
	...
	/**
	 * Controls sending "keep-alive" packets.  Positive parameter will be iterpreted  
	 * as number of miliseconds between "keep-alive" packets. Using 0 as a parameter
	 * will stop sending "keep-alive" packets.
	 * 
	 * @param interval interval between "keep-alive" packets in msec or 0
	 */
	public synchronized void setKeepAlive( int interval ) {
		...
		if( keepAliveInterval == 0 ) {
			// Do nothing
			// interval == 0 - means stop sending "keep-alive"
		} else if( keepAliveInterval > 0 ) {
			sendKeepAlive();
		}
	}
	
	private final static byte[]			nullPayload = "keep alive".getBytes();
	private final static PacketIgnore 	nullPacket = new PacketIgnore(nullPayload);
	
	private synchronized void sendKeepAlive() {
		if( tm == null ) {
			// Do nothing.
			// tm == null means Connection was closed
		} else {
			try {
				tm.sendMessage( nullPacket.getPayload());
				keepAliveTimer = new Timer();
				keepAliveTimer.schedule( new TimerTask() 
					{public void run() {
							sendKeepAlive();
					}},
					keepAliveInterval * 1000 // sec -> msec
				);
			} catch (IOException e) {
				// Do nothing.  
				// It looks like sendMessage does "right things" before re-throw IOException
			}
		}
	}
	
	public long getSendBytes() {
		return tm == null ? 0 : tm.getSendBytes();
	}
	
	public long getRecievedBytes() {
		return tm == null ? 0 : tm.getRecievedBytes();
	}

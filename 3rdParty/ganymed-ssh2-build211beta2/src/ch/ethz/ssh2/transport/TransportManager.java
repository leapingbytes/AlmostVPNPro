...
public class TransportManager
{
	...
	private void establishConnection(ProxyData proxyData, int connectTimeout) throws IOException
	{
		if (proxyData == null)
		{
			...
			sock = proxyData.connect( hostname, port, connectTimeout);
		}
		
		if( _tcpNoDelay >= 0 ) {
			sock.setTcpNoDelay( _tcpNoDelay == 1 );
		}
		if( _soTimeout >= 0 ) {
			sock.setSoTimeout( _soTimeout );
		}
	}
	...
	public long getSendBytes() {
		return tc.getSendBytes();
	}
	
	public long getRecievedBytes() {
		return tc.getRecievedBytes();
	}
}

package org.unknown.socks.server;
...
import org.unknown.socks.InetRange;
...
//////////////////
  private int getRangeIndex(InetAddress ip){
     int index = 0;
     Enumeration hostsElements = hosts.elements();
     while(hostsElements.hasMoreElements()){
       InetRange ir = (InetRange) hostsElements.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration usersElements = ((Hashtable)users.elementAt(i)).keys();
    if(!usersElements.hasMoreElements()) return "";
    String s = usersElements.nextElement().toString();
    while(usersElements.hasMoreElements())
       s += "; "+usersElements.nextElement();
